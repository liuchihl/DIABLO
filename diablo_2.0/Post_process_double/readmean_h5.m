...
% base_dir='/glade/campaign/univ/uosc0024/diablo_2.0/Large_case/d_5/Ri';
% base_dir='/glade/campaign/univ/uosc0024/diablo_2.0/Large_case/d_10/Ri_0.12';
 base_dir=['/glade/campaign/univ/uosc0024/diablo_2.0/Large_case/d_4/boundary/Ri_0.12_0.1_3'];
% base_dir='/glade/scratch/liuchihl/temp/diablo/DIABLO-master/diablo_2.0/Large_case/d_3/Ri_0.16_0.5_20_dup';
% base_dir='/glade/scratch/liuchihl/temp/diablo/DIABLO-master/diablo_2.0/KH_test4'
NY=361;
N_TH=1;
...
%  NY=1081;%NY should match the value in grid_def.all
...
thv=zeros(NY,nk,N_TH); pe_diss=zeros(NY,nk,N_TH); k2d=zeros(NY,nk); k3d=zeros(NY,nk);
epsilon=zeros(NY,nk);
for k=1:nk
...
% lines here to read in the variables, following the examples below
    ...
    k2d(:,k)=h5read(filename,varname);
    varname=['/k3d/' timename];
    k3d(:,k)=h5read(filename,varname);
...
%% plots integraterd TKE, a test for seeing if the run is enough
% close all;
%figure; 
 hold on;
semilogy(time,tke_int/20,'linewidth',2);
% semilogy(time,tke_int,'linewidth',2);
% semilogy(time(1:903),tke_int(1:903),'linewidth',2);
%  le = legend('KICK=0.5','KICK=0.2');
ylabel('TKE','fontsize',12);
xlabel('Time','fontsize',12);
set(gcf,'color','w');
set(gca,'fontsize',12,'yminortick','on','xminortick','on');
%print -djpeg tke_t.jpg

%figure; plot(time,mean(ke2)); hold on; plot(time,mean(ke3));
%xlabel('t','fontsize',12); ylabel('K_{2d}, K_{3d}');
%set(gca,'fontsize',12,'yminortick','on','xminortick','on');
%print -djpeg tke_t.jpg
